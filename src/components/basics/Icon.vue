<template>
  <div class="basic-icon">
    <Header title="图标" bgColor="bg-gradual-blue"> </Header>
		<div class="cu-bar bg-white search fixed" style="top:1rem">
			<form class="search-form round">
				<span class="cuIcon-search"></span>
				<input placeholder="搜索cuIcon" type="search" v-model="inputIcon" @input="searchIcon" />
			</form>
		</div>
		<ul class="cu-list grid col-3" style="margin-top: 2.3rem;">
			<li class="cu-item" v-for="(item,index) in cuIcon" :key="index" v-if="item.isShow">
				<span :class="['lg','text-gray','cuIcon-' + item.name]"></span>
				<span class="icon-name" v-text="item.name"></span>
			</li>
		</ul>
  </div>
</template>
  ...
  data () {
	...
	methods:{
		searchIcon:function(e){
			let key = this.inputIcon.toLowerCase()
